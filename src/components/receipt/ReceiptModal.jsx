...
const ReceiptModal = ({ isOpen, onClose, onSubmit, initialData, fixedClientId, fixedClientName }) => {
  ...
  }, [isOpen, initialData, fixedClientId]);
  ...
  // Crear/actualizar recibo (Paso 1)
  const handleCreateOrUpdate = async (e) => {
    ...
    // Para creación nueva: devolvemos el created y activamos Paso 2 sin cerrar el modal
    const created = await onSubmit(receiptData);
    if (created && created.id) {
      setCreatedReceipt(created);      // 👉 Habilita el Paso 2 en este mismo modal
      fetchProducts();
      // fetchDetails se dispara por useEffect(createdReceipt)
    } else {
      console.error('No se obtuvo un recibo válido al crear:', created);
    }
  };

  // Handlers del Paso 2 (detalles)
  ...
  const handleQuantityChange = (e) => {
    const q = parseFloat(e.target.value); // permite decimales
    setQuantity(q);
  ...
  const handleAddDetail = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        unit_price: unitPrice !== '' ? Number(unitPrice) : undefined, // NO mandes amount
      };
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-5xl relative z-50">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-semibold mb-4">
            {initialData
              ? 'Editar Recibo'
              : (createdReceipt ? `Recibo #${createdReceipt?.nro || createdReceipt?.id}` : 'Crear Recibo')}
          </h2>
            ...
            <div className="mb-3">
              <p className="text-sm text-gray-600">
                ...
                <span className="font-semibold">
                  {createdReceipt.client?.name
                    || fixedClientName
                    || clients.find(c => Number(c.id) === Number(createdReceipt.id_client))?.name}
                </span>
              </p>
            </div>
